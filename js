...
if (window.location.href.includes('config')) {
  ...
  if (!window.location.search.includes('season=')) {
    ...
    window.location.search = `${previous[0]}&season=${previous[0].includes('time=all') ? 'current' : new Date('2019-01-08T05:00:00.000Z') > new Date(window.location.search.split('to')[1].split('&')[0]) ? 'one' : 'current'}${previous.length > 1 ? `&focus=${previous[1]}` : ''}`
  } else (async function () {
